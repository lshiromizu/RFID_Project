...
def distance_vs_RSSI(device):

    device.set_RF_mode(mode=3)
    device.set_power(pwr=30.0, save=False)
    device.set_antenna(ant_1=True, ant_2=True, ant_3=True, ant_4=True, save=False)
    ...
    for i in range(13):
        time.sleep(15)
        tag = device.read_start(10)
        RSSI.append(tag['RSSI'].max())
        distances.append(i*0.25)
        print("move tag ", i)

    # Plot distance vs reading power
...
def pwr_vs_time(device):
    ...
    for i in range(1, 30 + 1, 1):
        ...
        tag = device.read_start(100)
        seconds.append(time.time() - start_time)

    # Plot distance vs reading power
    plt.plot(Read_Power, seconds)
    plt.xlabel("Reader Output Power (dBm)")
    plt.ylabel("Time (seconds)")
    plt.title('Reader Output Power vs. Time')
    plt.grid(True)
    plt.show()
